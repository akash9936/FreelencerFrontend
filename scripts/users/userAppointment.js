...
async function dashboard() {
    ...
    let res;
    try {
        res = await fetch(`${window.API_CONFIG.BASE_URL}/appointment/`, {
            method: "GET",
            headers: {
...
function display1(out) {
    ...
    async function updatestatus(data, data1, apid) {
        ...
        try {
            const res = await fetch(`${window.API_CONFIG.BASE_URL}/appointment/reject/${apid}`, {
...
date_filter.addEventListener('change', async () => {
    ...
    const res = await fetch(`${window.API_CONFIG.BASE_URL}/appointment?date=${date}`, {
        headers: {
            Authorization: token
        }
    })
