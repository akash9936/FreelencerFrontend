...
async function update1(obj, id) {
    ...
    try {
        let res = await fetch(`${window.API_CONFIG.BASE_URL}/users/update/${id}`, {
...
async function del1(id) {
    console.log(id);
    try {
        let res = await fetch(`${window.API_CONFIG.BASE_URL}/users/delete/${id}`, {
...
async function userdata(id) {
    ...
    try {
        let out = await fetch(`${window.API_CONFIG.BASE_URL}/users/data/${id}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                 Authorization:token
